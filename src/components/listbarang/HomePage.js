...
const ListBarang = ({ setFresh, theme }) => {
  ...
  const handleDelete = async () => {
    const response = await API.delete(`/deleteList/${idBarang}`);
    setOpen(false);
    refetch();
    setFresh(true);
  };
            ...
            {DataBarang?.length !== 0 ? (
              <Paper align="center" sx={{ maxWidth: "100%" }}>
                <div className={cssModules.listItem}>
                  ...
                  <span className={cssModules.h5}>Item list</span>
                </div>
                <Table
                  sx={{ width: "100%" }}
